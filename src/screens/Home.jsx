  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '16px 16px 14px', border: '1.5px solid #F0F0F0' }}>
      <div style={{ background: '#e8eef1', borderRadius: 8, padding: '12px', textAlign: 'center', marginBottom: 20 }}>
        <div style={{ fontSize: 28, fontWeight: 800, color: '#2F4156', letterSpacing: '-0.02em' }}>{fmtDuration(value)}</div>
      </div>
      <div ref={trackRef} style={{ position: 'relative', height: 6, background: '#F0F0F0', borderRadius: 3, margin: '0 11px 14px' }}>
        <div style={{ position: 'absolute', left: 0, width: `${pct}%`, top: 0, bottom: 0, background: '#2F4156', borderRadius: 3 }} />
        <div onMouseDown={startDrag} onTouchStart={startDrag} style={{ position: 'absolute', left: `calc(${pct}% - 11px)`, top: -8, width: 22, height: 22, borderRadius: 11, background: '#2F4156', border: '3px solid #fff', boxShadow: '0 1px 6px rgba(138,157,168,0.45)', cursor: 'grab', zIndex: 2, touchAction: 'none' }} />
      </div>
    ...
    <div style={{ background: '#fff', borderRadius: 12, padding: '16px 16px 14px', border: '1.5px solid #F0F0F0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        <div style={{ flex: 1, background: '#e8eef1', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: '#2F4156', letterSpacing: '-0.02em' }}>{fmtSlot(startSlot)} <span style={{ fontSize: 13, fontWeight: 600 }}>{fmtPeriod(startSlot)}</span></div>
        </div>
        <div style={{ color: '#CCC', fontSize: 20 }}>→</div>
        <div style={{ flex: 1, background: '#e8eef1', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 800, color: '#2F4156', letterSpacing: '-0.02em' }}>{fmtSlot(endSlot)} <span style={{ fontSize: 13, fontWeight: 600 }}>{fmtPeriod(endSlot)}</span></div>
        </div>
      </div>
      <div ref={trackRef} style={{ position: 'relative', height: 6, background: '#F0F0F0', borderRadius: 3, margin: '0 11px 14px' }}>
        <div style={{ position: 'absolute', left: `${sPct}%`, width: `${ePct - sPct}%`, top: 0, bottom: 0, background: '#2F4156', borderRadius: 3 }} />
        <div onMouseDown={startDrag('start')} onTouchStart={startDrag('start')} style={{ position: 'absolute', left: `calc(${sPct}% - 11px)`, top: -8, width: 22, height: 22, borderRadius: 11, background: '#2F4156', border: '3px solid #fff', boxShadow: '0 1px 6px rgba(138,157,168,0.45)', cursor: 'grab', zIndex: 2, touchAction: 'none' }} />
        <div onMouseDown={startDrag('end')} onTouchStart={startDrag('end')} style={{ position: 'absolute', left: `calc(${ePct}% - 11px)`, top: -8, width: 22, height: 22, borderRadius: 11, background: '#2F4156', border: '3px solid #fff', boxShadow: '0 1px 6px rgba(138,157,168,0.45)', cursor: 'grab', zIndex: 2, touchAction: 'none' }} />
      </div>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, border: '1.5px solid #F0F0F0', overflow: 'hidden', marginBottom: 16 }}>
      <div style={{ padding: '10px 16px 8px', background: '#F8FFFE', borderBottom: '1px solid #F0F0F0', display: 'flex', alignItems: 'center', gap: 6 }}>
        <div style={{ width: 6, height: 6, borderRadius: 3, background: phase === 'done' || phase === 'dragging' ? '#2F4156' : '#FFB300', flexShrink: 0 }} />
        <span style={{ fontSize: 12, fontWeight: 600, color: phase === 'done' || phase === 'dragging' ? '#2F4156' : '#F57F17' }}>{phaseLabel}</span>
      ...
      <div style={{ padding: '12px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <button onClick={prevMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 6px', color: '#2F4156' }}><IcChevron dir="left" size={16} /></button>
          <span style={{ fontSize: 14, fontWeight: 700, color: '#2F4156' }}>{MONTH_NAMES[viewMonth]} {viewYear}</span>
          <button onClick={nextMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 6px', color: '#2F4156' }}><IcChevron dir="right" size={16} /></button>
        </div>
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
            ...
            return (
              <div key={d} data-day={d} onMouseDown={() => handleMouseDown(d)} onMouseEnter={() => handleMouseEnter(d)} onTouchStart={() => handleMouseDown(d)} style={{ position: 'relative', height: 36, cursor: disabled ? 'default' : 'pointer' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, width: 32, height: 32, borderRadius: 16, margin: '2px auto 0', display: 'flex', alignItems: 'center', justifyContent: 'center', background: isStart || isEnd ? '#2F4156' : 'transparent', border: isToday && !isStart && !isEnd ? '1.5px solid #2F4156' : 'none', fontSize: 13, fontWeight: isStart || isEnd || inRange ? 600 : 400, color: isStart || isEnd ? '#fff' : disabled ? '#DDD' : isToday ? '#2F4156' : inRange ? '#2F4156' : '#2F4156' }}>{d}</div>
              </div>
            );
          })}
        </div>
    ...
    <div className="bottom-sheet-overlay" onClick={onClose}>
      <div className="bottom-sheet" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#2F4156' }}>回覆截止日</div>
          {selected && <div style={{ fontSize: 13, fontWeight: 600, color: '#2F4156' }}>{formatDate(selected)}</div>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <button onClick={prevMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', color: '#2F4156' }}><IcChevron dir="left" size={18} /></button>
          <span style={{ fontSize: 15, fontWeight: 700, color: '#2F4156' }}>{MONTH_NAMES[viewMonth]} {viewYear}</span>
          <button onClick={nextMonth} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', color: '#2F4156' }}><IcChevron dir="right" size={18} /></button>
        </div>
          ...
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
            ...
            return (
              <div key={d} onClick={() => { onSelect(new Date(viewYear, viewMonth, d)); onClose(); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 42, cursor: 'pointer' }}>
                <div style={{ width: 36, height: 36, borderRadius: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', background: sel ? '#2F4156' : 'transparent', border: todayMark && !sel ? '1.5px solid #2F4156' : 'none', fontSize: 14, fontWeight: sel || todayMark ? 700 : 400, color: sel ? '#fff' : todayMark ? '#2F4156' : '#2F4156' }}>{d}</div>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ padding: '16px 16px 0' }}>
          ...
          <div className="form-field">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div onClick={() => setAllDay(v => !v)} style={{ width: 40, height: 24, borderRadius: 12, background: allDay ? '#2F4156' : '#E0E0E0', position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0 }}>
          ...
          <div className="form-field">
            ...
            <div style={{ position: 'relative' }} onClick={() => setShowDeadlineSheet(true)}>
              <div className="form-input" style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', color: deadlineDate ? '#2F4156' : '#BEC0C4', paddingRight: 36 }}>
